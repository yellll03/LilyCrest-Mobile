...
// (MOB-P0-01). POST /api/m/users/documents previously trusted a
// client-supplied `storagePath` outright: any authenticated tenant could
// register another tenant's real object path (or any path in the bucket)
// under their own document metadata, and the later GET .../content and
// DELETE endpoints would use privileged Firebase Admin access against that
// unverified path. This module is the single place that proves a storage
// object, its download URL, and the configured bucket all identify the
// exact same object under the authenticated tenant's own upload prefix —
// used both at metadata-registration time and again before every
// privileged read/delete of already-stored metadata (which may predate
// this invariant).
...
// download URL shape (the exact format the backend's own /upload/firebase-
// storage route issues) can be decoded into a verifiable bucket/object pair.
// Any other host, path shape, or malformed encoding returns null so callers
// fail closed instead of guessing at an unfamiliar URL format.
