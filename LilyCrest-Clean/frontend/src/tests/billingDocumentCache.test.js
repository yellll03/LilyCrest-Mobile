import { billingDocumentCacheKey, normalizeStatementVersion } from '../utils/billingDocumentCache';
...
describe('billing statement_version cache lifecycle', () => {
  ...
  it('uses stable, distinct statement and receipt cache keys', () => {
    const bill = { statement_version: 7 };
    expect(billingDocumentCacheKey('bill-1', bill)).toBe('bill-1_statement_v7');
    expect(billingDocumentCacheKey('bill-1', bill, 'receipt')).toBe('bill-1_receipt_v7');
  });
